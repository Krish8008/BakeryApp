  ...
  if (!cake) {
    return (
      <div className="min-h-screen flex justify-center items-center bg-pink-50">
        <h2 className="text-2xl font-semibold text-pink-600">Loading Cake...</h2>
      </div>
    );
  }
      ...
      <div className="max-w-7xl mx-auto bg-white rounded-3xl shadow-xl overflow-hidden">
        <div className="grid lg:grid-cols-2 gap-10 p-6 md:p-10">
          ...
          <div className="flex flex-col justify-center">
            ...
            <h1 className="text-5xl font-bold mt-4">{cake.name}</h1>

            <p className="text-gray-600 mt-5 leading-relaxed">{cake.description}</p>

            <h2 className="text-4xl font-bold text-pink-600 mt-6">₹{cake.price}</h2>

            <div className="grid grid-cols-2 gap-4 mt-8">
              <div className="bg-gray-100 p-4 rounded-xl">
                <p className="text-gray-500 text-sm">Weight</p>
                <h3 className="font-semibold">{cake.weight}</h3>
              </div>

              <div className="bg-gray-100 p-4 rounded-xl">
                <p className="text-gray-500 text-sm">Flavor</p>
                <h3 className="font-semibold">{cake.flavor}</h3>
              </div>
            </div>

            {cake.eggless && (
              <div className="mt-6">
                <span className="bg-green-100 text-green-700 px-4 py-2 rounded-full">✅ Eggless Cake</span>
              </div>
            )}

            <div className="flex gap-4 mt-10 flex-wrap">
              <button className="flex-1 bg-pink-600 hover:bg-pink-700 text-white py-3 rounded-xl font-semibold">
                Add to Cart
              </button>
